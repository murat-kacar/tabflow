...
- Write into the active `docs/` tree. For a major rewrite, open a
  parallel draft tree (convention: `docs.next/`) and use the swap
  protocol below to promote it.
...
- Link with relative paths so the tree stays valid when rendered on any
  host. Do not hard-code the tree root (for example a draft suffix or
...
When a draft rewrite tree is locked, the swap commit moves the active
`docs/` to `docs.archive/` and promotes the draft tree to `docs/` in
one commit.

Before that commit:

- Verify that every relative link inside the draft tree resolves.
- Audit the draft tree for any reference to its own draft path
  (for example `docs.next/...`) and rewrite those references as plain
